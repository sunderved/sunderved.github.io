...
function drawArc(va, vb, vw)
{
	...
	if (a<b) {
		...
		var length = 50;
		var left = 60+x*length+(isEven(y)?0:length/2); // left
		var top  = 40+y*length*(Math.sqrt(3)/2);       // top
		...
		var e = createDiv('container', arcname(va, vb), left, top, 'track track'+vw+' rot'+rot, clickedTrack);
		e.VA = va;
		e.VB = vb;		
	}
}
...
function drawCity(city)
{
	...
	createDiv('container', city, left-7, top-7, 'city', clickedCity);
}
...
function createMap()
{
	for (va in g.vertices) {
		for (vb in g.vertices[va]) {
			drawArc(va, vb, g.vertices[va][vb]);
		}
	}
