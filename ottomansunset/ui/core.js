...
{
	document.getElementById('waterS4').style.vibility = 'hidden';		
	document.getElementById('waterS5').style.vibility = 'hidden';
}
...
function UI_updateCounters()
{
	...
	if (game.StraitsClosed) {
		document.getElementById('Narrows').classList = 'closed';
	}		
	...
	for (defense in game.Narrows) {
		...
		if (game.Narrows[defense]==false) {
			...
			if ((defense=='Minefield_1') || (defense=='Minefield_2')) {
				document.getElementById(defense).classList = 'defense mine';
			} else {
				document.getElementById(defense).classList = 'defense gun';
			}
		}
	}
}
...
{	
	document.getElementById(front).innerHTML = game.Army[front];	
	var pos = game.Front[front];
	if (pos!=undefined) {
		if (pos>0) {
			document.getElementById(front).classList = 'army '+front+''+pos;	
		} else {
			document.getElementById(front).classList = 'army Constantinople';	
		}
	} else {
		document.getElementById(front).classList = 'army inactive';
	}
...
{	
	for (army in game.Front) {
		if ( CanAttackFront(army) ) {
			UI_enable(army);	
		}
	}	

	for (theatre in game.Theatre) {
	...
	if ( CanFortifyNarrows() ) {
		UI_enable('Narrows');	
		for (defense of ['Minefield_1', 'Yildiz', 'Dardanos', 'Minefield_2', 'Nagara']) { 
			if (game.Narrows[defense]==false) { 
				UI_enable(defense); 
			}
		}					
...
function UI_showDieSuccess(success) {
	if ((success==true) || (success=='victory')) {
	  die.div.classList = 'success';
  } else if ((success==false) || (success=='defeat')) {
	  die.div.classList = 'failure';
  } else {
	  die.div.classList = '';
	}	  
}
